...
def _add_item(catalog: pystac.Catalog, item: pystac.Item, raster: str) -> None:
...
def _create_catalog(item_urls: tuple[str, ...], water_bodies: tuple[str, ...]) -> None:
    if not item_urls or len(item_urls) != len(water_bodies):
        raise ValueError("Provide the same nonzero number of STAC items and water body rasters")
...
def execute(*, item_urls: tuple[str, ...], water_bodies: tuple[str, ...]) -> None:
    """Create catalog.json and one item directory per water mask."""
    try:
        logger.info("Creating catalog for {} water masks", len(water_bodies))
        _create_catalog(item_urls, water_bodies)
