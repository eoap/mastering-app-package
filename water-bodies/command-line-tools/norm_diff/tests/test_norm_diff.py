...
def test_normalized_difference(raster_factory, tmp_path, monkeypatch):
    a = raster_factory("a.tif", [[3, 0], [1, 4]])
    b = raster_factory("b.tif", [[1, 0], [3, 4]])
    monkeypatch.chdir(tmp_path)
    result = CliRunner().invoke(norm_diff, ["--rasters", a, "--rasters", b])
    assert result.exit_code == 0, result.output
    with rasterio.open("norm_diff.tif") as output:
        np.testing.assert_allclose(
            output.read(1), [[0.5, np.nan], [-0.5, 0]], equal_nan=True
        )
...
def test_single_raster(raster_factory):
    raster = raster_factory("one.tif", [[1, 2]])
    result = CliRunner().invoke(norm_diff, ["--rasters", raster])
    assert result.exit_code == 1
