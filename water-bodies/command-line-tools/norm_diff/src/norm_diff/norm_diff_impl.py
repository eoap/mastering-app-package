...
def _calculate(rasters: tuple[str, ...]) -> tuple[np.ndarray, dict]:
    if len(rasters) != 2:
    ...
    with rasterio.open(rasters[0]) as first, rasterio.open(rasters[1]) as second:
        ...
        if grid1 != grid2:
            raise ValueError("Input rasters must have matching dimensions, transform, and CRS")
...
def execute(*, rasters: tuple[str, ...]) -> None:
